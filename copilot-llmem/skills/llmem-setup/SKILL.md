...
Clone the LLMem repository, then run the setup script:

```bash
git clone https://github.com/<owner>/LLMem.git /tmp/llmem-repo
cd /tmp/llmem-repo && ./setup.sh --plugin <platform>
```
...
```bash
curl -sSL https://raw.githubusercontent.com/<owner>/LLMem/main/setup.sh | bash
```
...
**Skills not discovered** — Verify the skill directories exist: `ls ~/.agents/skills/`. If missing, re-run the plugin install step. For OpenCode: `cd opencode-llmem && npm install`. For Copilot: reinstall the `copilot-llmem` plugin.