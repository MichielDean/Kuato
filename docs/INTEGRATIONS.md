...
The `copilot-llmem` package provides a Copilot CLI plugin that integrates LLMem session hooks into GitHub Copilot CLI via declarative hooks. It calls the same `llmem` CLI under the hood as the OpenCode plugin.

### Installation

Install directly from the GitHub repository using Copilot CLI's plugin system:

```bash
copilot plugin install MichielDean/LLMem:copilot-llmem
```

Or, if you've cloned the repo, install from the `copilot-llmem/` directory:
...
The `postinstall` script copies the skill directories (`llmem`, `introspection`, `introspection-review-tracker`) and the `memory-assistant` agent to `~/.agents/` so they are discoverable at runtime. You still need the `llmem` CLI installed (from the repo root: `pip install .`).
