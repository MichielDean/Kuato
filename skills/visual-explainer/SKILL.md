...
Detailed prompt templates in `./commands/`. In OpenCode, these are namespaced (`/visual-explainer:diff-review`). In other agents, invoke `$visual-explainer` and describe the workflow.
...
**Example:**
```bash
bash {{skill_dir}}/scripts/share.sh ./diagrams/my-diagram.html
...
**Requirements:**
- vercel-deploy skill (should be pre-installed; if not, install it)
