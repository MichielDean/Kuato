...
description: >
  Adversarial code reviews with outside-view self-assessment and zero tolerance for mediocrity.
  ...
  structured feedback with severity tiers (Blocking, Required, Strong Suggestions, Noted) and
...
### 7. Outside-View Review

When reviewing your own code, the introspection illusion makes you trust your reasoning instead of checking your output. This section is a specific cognitive debiasing technique (outside-view self-assessment, Pronin 2007), not a vague reminder to "be careful." Treat your own output as if someone else wrote it.

Answer these four questions before submitting any self-review:

1. **If someone else wrote this, would I flag these issues or let them slide?** — Compare your review of your own code against the standards you apply to others. If you'd flag a missing error path in a colleague's PR but accept "it should work" in your own, that's the introspection illusion.

2. **Am I giving myself passes I'd flag in a PR review — assuming intent, accepting 'good enough', skipping edge cases?** — List every concession you made. "This edge case probably won't happen" is a pass you'd reject from anyone else.

3. **What would an adversarial reviewer find that I'm not seeing?** — Read each function assuming the worst case inputs. Trace every error path. Check every assertion. If you wrote it, your brain auto-completes the intended behavior — read what's actually there.

4. **Where am I trusting my own reasoning instead of verifying observable behavior?** — Run the tests. Read the output. Open the endpoint. Check the actual HTTP status. Don't assume your code does what you intended — verify what it actually does.
...
2. **Required**: Slop, lazy patterns, unhandled edge cases, poor naming, type safety violations
...
When the review is attached to a pull request, offer the option to submit your review verbatim as a PR comment. Include attribution at the top: "Review feedback assisted by the [critical-code-reviewer skill](https://github.com/opencode-kuato/llmem/skills/critical-code-reviewer/SKILL.md)."

**Record review findings for future reference:**

After every review, findings should be recorded for future reference and calibration. This is a mechanical post-review step — the agent MUST do it after every review invocation, regardless of whether the review was clean or had findings.

**For each review with findings**, save a structured record of each finding with: category, description, and severity.

**For a clean review (no findings)**, record that the review passed with no issues.

This step MUST produce at least one record per review invocation — one per finding for non-clean reviews, or a clean-review record for clean reviews. Failure to create any record is a bug, not an acceptable condition.
